...
public class DummyAngularHUDController : MonoBehaviour {

	private DummyPlayerController dpc;
	...
	void Start () {
		dpc = GetComponent<DummyPlayerController> ();

		angularHUD = GameObject.Find ("Dummy Angular HUD");
		angularVelocityArrow = GameObject.Find ("Dummy Angular Velocity Arrow").transform;
		angularVelocityArrow2 = GameObject.Find ("Dummy Angular Velocity Arrow 2").transform;
		//angularMomentumArrow = GameObject.Find ("Dummy Angular Momentum Arrow").transform;

		mOIRingLineRenderer = GetComponentInChildren<LineRenderer> ();
		mOIRingLineRenderer.SetVertexCount (circlePoints + 1);

		maxMOI = dpc.GetMaxMomentOfInertia ();
	}

	void Update () {

		Vector2 cOM = dpc.GetCentreOfMass ();
		...
		float mOI = dpc.GetMomentOfInertia ();
		float angularVelocity = dpc.GetAngularVelocity ();
		...
		//angularMomentumArrow.Rotate (0, 0, (-angularVelocity + angularMomentum) * Time.deltaTime);
